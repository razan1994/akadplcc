...
namespace App\Http\Controllers\Frontend\Radiology;

use App\Http\Controllers\Controller;
use App\Http\Requests\Frontend\Radiology\RadiologyStoreImageFormRequest;
use App\Http\Requests\Frontend\Radiology\RadiologyUpdateProfileFormRequest;
use App\Http\Requests\Frontend\Radiology\RadiologyUpdateWeekPlanFormRequest;
use App\Models\RadiologyCenterGallery;
use App\Models\RadiologyCenterWeekPlan;
...
class RadiologyController extends Controller
{
    ...
    function dashboard(Route $route,$active = null)
    {
        try {
            ...
            return view('front_end_inners.radiology_centers.radiology_dashboard', compact('auth','active'));
        } catch (\Throwable $th) {
    ...
    function radiologyUpdateProfile(RadiologyUpdateProfileFormRequest $request,$id,Route $route)
    {
        try {
            if(!Auth::guard('radiology_center')->check()){
            ...
            $user = Auth::guard('radiology_center')->user();
            ...
            return redirect()->route('radiology_center.radiology-dashboard','radiologyUpdateProfile')->with('success','updated successfully');
    ...
    function updateRadiologyWeekPlan(RadiologyUpdateWeekPlanFormRequest $request,$id,Route $route){
        try {
            ...
            $radiology_center = Auth::guard('radiology_center')->user();

            $week_plan = RadiologyCenterWeekPlan::where('radiology_center_id',$radiology_center->id)->get()->first();
            ...
            if($week_plan){
                ...
                $data['radiology_center_id'] = $radiology_center->id;
                DB::transaction(function () use ($data,$week_plan) {
                    RadiologyCenterWeekPlan::create($data);
                });
            }

            return redirect()->route('radiology_center.radiology-dashboard','radiologyWeekPlan')->with('success','Week Plan Updated Successfully');
    ...
    function radiologyStoreImages(RadiologyStoreImageFormRequest $request,Route $route){
        try {
            ...
            if (isset($request->image)) {
                $request_data = [
                    'radiology_center_id' => $radiology_center->id,
                ];
                foreach ($request->image as $key => $value) {
                    $orginal_image = $value;
                    $upload_location = 'storage/radiology_center_gallery/';
                    $original_name = $orginal_image->getClientOriginalName();
                    $file_name = $this->saveFileWithOriginalName('radiology_center_galleries', 'image', $orginal_image, $original_name, $upload_location);
                    $request_data['image'] = $file_name;
                    DB::transaction(function () use ($request_data) {
                        RadiologyCenterGallery::create($request_data);
                    });
                }
            } else {
            ...
            return redirect()->route('radiology_center.radiology-dashboard','gallery')->with('success','Images Added Successfully');
    ...
    function radiologyDeleteImage($id,Route $route){
        try {
            if(!Auth::guard('radiology_center')->check()){
                return redirect()->route('welcome')->with('danger','You Are Not Autherized');
            }

            $image = RadiologyCenterGallery::find($id);
            if($image){
                ...
                return redirect()->route('radiology_center.radiology-dashboard','gallery')->with('success','Image Deleted Succesfully');
            }else{
                return redirect()->route('radiology_center.radiology-dashboard','gallery')->with('danger','Image Not Found');
            }
        } catch (\Throwable $th) {
            $function_name =  $route->getActionName();
            $check_old_errors = new SupportTicket();
