...
class LatestNewsController extends Controller
{
    ...
    public function index(Request $request, Route $route)
    {
        ...
        try {
            $news_blogs = new LatestNew();
            $news_blogs = $news_blogs->select('*')->orderBy('created_at', 'asc')->get();

            return view('admin.latest_news.index', compact(
                'news_blogs',
            ));
        } catch (\Throwable $th) {
    ...
    public function store(StoreBlogFormRequest $request, Route $route)
    {
        try {
            ...
            DB::transaction(function () use ($created_data) {
                $base = LatestNew::create($created_data);
            });
    ...
    {
        try {
            $news_blog = LatestNew::find($id);

            if ($news_blog) {
                // return $news_blog;
                return view('admin.latest_news.show', compact('news_blog'));
            } else {
                return redirect()->route('super_admin.latest_news-index')->with('danger', 'This record is not in the records');
            }
        } catch (\Throwable $th) {
            $function_name =  $route->getActionName();
            $check_old_errors = new SupportTicket();
    ...
    public function edit($id, Route $route)
    {
        try {
            ...
            if ($news_blog) {
                return view('admin.latest_news.edit', compact('news_blog'));
            } else {
                return redirect()->route('super_admin.latest_news-index')->with('danger', 'This record is not in the records');
            }
        } catch (\Throwable $th) {
        ...
        try {
            ...
            if ($news_blog) {
                ...
                DB::transaction(function () use ($update_data, $id) {
                    LatestNew::find($id)->update($update_data);
                });
                return redirect()->back()->with('success', 'The data has been successfully updated');
            } else {
                return redirect()->route('super_admin.latest_news-index')->with('danger', 'This record does not exist in the records');
            }
        } catch (\Throwable $th) {
            $function_name =  $route->getActionName();
            $check_old_errors = new SupportTicket();
    ...
    public function softDelete($id, Route $route)
    {
        try {
            ...
            if ($news_blog) {
                ...
                return redirect()->route('super_admin.latest_news-index')->with('success', 'The deletion process has been successful');
            } else {
                return redirect()->route('super_admin.latest_news-index')->with('danger', 'This record is not in the records');
            }
        } catch (\Throwable $th) {
    ...
    public function showSoftDelete(Request $request, Route $route)
    {
        try {
            ...
            // return $news_blogs;
            return view('admin.latest_news.trashed', compact(
                'news_blogs',
            ));
        } catch (\Throwable $th) {
            $function_name =  $route->getActionName();
    ...
    {
        try {
            $news_blog = LatestNew::onlyTrashed()->find($id);
            if ($news_blog) {
                DB::transaction(function () use ($news_blog) {
                    $news_blog->restore();
                });
                return redirect()->route('super_admin.latest_news-index')->with('success', 'Restore Completed Successfully');
            } else {
                return redirect()->route('super_admin.latest_news-index')->with('danger', 'This section does not exist in the records');
            }
        } catch (\Throwable $th) {
            $function_name =  $route->getActionName();
            $check_old_errors = new SupportTicket();
